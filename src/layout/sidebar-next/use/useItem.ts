...
export default function useItem (props:any) {
  ...
  const isLinkActive = (item:any, exact?:any) => {
  ...
  const emitMobileItem = (event:any, itemEl:any) => {
  ...
  const onExpandEnter = (el:any) => {
    el.style.height = el.scrollHeight + 'px'
  }

  const onExpandAfterEnter = (el:any) => {
  ...
  const onExpandBeforeLeave = (el:any) => {
    if (isCollapsed.value && isFirstLevel.value) {
