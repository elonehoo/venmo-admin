...
export const initSidebar = (props:any, context:any) => {
  ...
  const updateIsCollapsed = (val:any) => {
    isCollapsed.value = val
  }

  const updateActiveShow = (id:any) => {
    activeShow.value = id
  }

  const setMobileItem = ({ item, itemEl }:any) => {
  ...
  const updateMobileItem = (item:any) => {
    mobileItem.item = item
  }

  const updateMobileItemRect = ({ top, height, padding, maxWidth, maxHeight }:any) => {
